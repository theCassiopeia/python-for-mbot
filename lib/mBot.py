...
class mBot():
	def __init__(self):
		...
		def doLEDMXclock(self,port):
				b = bytearray([0xff,0x55,0x8,0x0,0x2,0x29,port,0x3,0x1,time.localtime().tm_hour,time.localtime().tm_min])
				self.__writePackage(b)
		...
		def doLEDMXstring(self,port,x,y,LEDstring):
				print LEDstring
				b = bytearray([0xff,0x55,8+len(LEDstring),0x0,0x2,0x29,port,0x1,x,7-y,len(LEDstring)])
				b.extend(LEDstring)
				#print map(hex,b)
				self.__writePackage(b)
		...
		def requestVersion(self,extID,callback):
				self.__doCallback(extID,callback)
				self.__writePackage(bytearray([0xff,0x55,0x4,extID,0x1,0x0,0x0]))
